...
class DoubleDQN(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _compute_y_and_t(self, exp_batch, gamma):
        ...
        # 内部報酬を追加，ついでに学習
        ## バッチ内の次状態の予測モデルをつくる
        state_action = cp.array([list(s)+list(a) for s,a in zip(batch_state,
            [np.eye(2)[int(a)] for a in exp_batch['action']])],dtype=cp.float32)
        for i in range(self.loop):
            ...
            if i == 0:
                ...
                filt_f = (losses_a>self.delta_a).reshape(32,1).astype(np.float32)
                filt_a = (losses>self.delta_f).reshape(32,1).astype(np.float32)
            losses_a_itr = F.sum((y_a-u)*(y_a-u),axis=1).reshape(32,1)
            losses_itr = F.sum((y-t)*(y-t),axis=1).reshape(32,1)
            loss = F.mean_absolute_error(losses_itr, cp.full_like(losses_itr, 0.).astype(cp.float32))
            loss_a = F.mean_absolute_error(losses_a_itr, cp.full_like(losses_a_itr, 0.).astype(cp.float32))
        ...
        # self.record['gated_losses'].append(filt_a*filt_f)
        # self.record['target'].append(t.T[2])
        with chainer.no_backprop_mode():
            batch_q_target = F.reshape(
                self._compute_target_values(exp_batch, gamma),
                (batch_size, 1))
            self.record['qt+i_r'].append(batch_q_target + (filt_a*filt_f/self.std))
        return batch_q, batch_q_target + (filt_a*filt_f/self.std) # TODO:分散の補正は後でする
