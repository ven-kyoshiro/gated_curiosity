...
def main():
    ...
    for i in range(1, n_episodes + 1):
        ...
        if i % 100 == 0 or i == n_episodes-1:
            with open('po_logs/po_gate_mindel3'+str(i)+'_std'+str(agent.std)+'_delta'+str(agent.delta)+'.pickle',mode='wb') as f:
                pickle.dump(agent.record,f)
            notify('po_gate_mindel3\n : episode:'+str(i)+' R:'+str(R))
            for k in agent.record.keys():
                agent.record[k] = []
        agent.stop_episode_and_train(obs, reward, done)
    agent.save('po_logs/po_gate_mindel3')
    print('Finished.')
