...
from gpu_gated_curio_dqn_noa import DoubleDQN
...
def main():
    ...
    for i in range(1, n_episodes + 1):
        ...
        if i % 100 == 0 or i == n_episodes-1:
            with open('gpu_logs/gpu_gate1_'+str(i)+'_std'+str(agent.std)+'_delta'+str(agent.delta)+'.pickle',mode='wb') as f:
                pickle.dump(agent.record,f)
            notify('gpu_gate1\n : episode:'+str(i)+' R:'+str(R))
            for k in agent.record.keys():
                agent.record[k] = []
        agent.stop_episode_and_train(obs, reward, done)
    agent.save('gpu_logs/gpu_gate1')
    print('Finished.')
